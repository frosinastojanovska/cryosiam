...
from cryosiam.apps.prototype_matching import PrototypeMatchingModule


def main(config_file_path):
    if not torch.cuda.is_available():
        raise RuntimeError("CryoSiam requires a CUDA-enabled PyTorch for training."
    ...
    if 'pretrained_model' not in cfg:
        checkpoint = torch.load(cfg['pretrained_dense_simsiam_model'], weights_only=False)
        dense_backbone_config = checkpoint['hyper_parameters']['config']
        net = PrototypeMatchingModule(cfg, dense_backbone_config)
    else:
        # initialise the LightningModule
        checkpoint = torch.load(cfg['pretrained_model'], weights_only=False)
        dense_backbone_config = checkpoint['hyper_parameters']['dense_backbone_config']
        net = PrototypeMatchingModule(cfg, dense_backbone_config)
    ...
    checkpoint_callback = ModelCheckpoint(dirpath=os.path.join(cfg['log_dir'], 'model'),
                                          ...
                                          save_top_k=1,
                                          verbose=True)
    last_checkpoint = ModelCheckpoint(dirpath=os.path.join(cfg['log_dir'], 'model'),
                                      filename="last_model",
                                      every_n_epochs=cfg['hyper_parameters']['val_interval'],
                                      save_top_k=0,
                                      save_last=True,
